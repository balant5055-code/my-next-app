...
export default function EditEventPage() {
  ...
  const removeCategory = (index: number) => {
    const updated = event.categories.filter((_: any, i: number) => i !== index);
    setEvent({ ...event, categories: updated });
  };
    ...
    <div className="max-w-6xl space-y-12">
      ...
      <Section title="Basic Details">
        <AnimatedInput
          ...
          icon={<TagIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="slug"
          value={event.slug}
          onChange={handleChange}
          icon={<LinkIcon className={RED_ICON} />}
        />
        <AnimatedInput
          type="date"
          name="date"
          value={event.date}
          onChange={handleChange}
          icon={<CalendarDaysIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="city"
          value={event.city}
          onChange={handleChange}
          icon={<MapPinIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="venue"
          value={event.venue}
          onChange={handleChange}
          icon={<BuildingOfficeIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="mapLink"
          value={event.mapLink}
          onChange={handleChange}
          icon={<MapIcon className={RED_ICON} />}
        />
        <AnimatedInput
          type="time"
          name="gateOpen"
          ...
          onChange={handleChange}
          icon={<ClockIcon className={RED_ICON} />}
        />
        <AnimatedInput
          type="time"
          name="raceStart"
        ...
        <AnimatedInput
          ...
          onChange={handleChange}
          icon={<PhotoIcon className={RED_ICON} />}
        />

        {event.bannerURL && (
          <img
            src={event.bannerURL}
            className="h-48 rounded-xl mt-4 object-cover"
          />
        )}
      </Section>
      ...
      <Section title="Organizer">
        <AnimatedInput
          ...
          onChange={handleChange}
          icon={<UserIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="organizerEmail"
          value={event.organizerEmail}
          onChange={handleChange}
          icon={<EnvelopeIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="organizerPhone"
          value={event.organizerPhone}
          onChange={handleChange}
          icon={<PhoneIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="supportEmail"
      ...
      <Section title="Social Links">
        <AnimatedInput
          ...
          onChange={handleChange}
          icon={<GlobeAltIcon className={RED_ICON} />}
        />
        <AnimatedInput
          name="facebook"
        ...
        {event.categories.map((cat: any, i: number) => (
          <div key={i} className="border rounded-xl p-4 space-y-2 bg-gray-50">
            ...
            <AnimatedInput
              name={`price-${i}`}
              value={cat.price}
              onChange={(e) => handleCategoryChange(i, "price", e.target.value)}
              icon={<CurrencyRupeeIcon className={RED_ICON} />}
            />
            <AnimatedInput
              name={`minAge-${i}`}
              value={cat.minAge}
            ...
            <AnimatedInput
              ...
              icon={<UsersIcon className={RED_ICON} />}
            />
